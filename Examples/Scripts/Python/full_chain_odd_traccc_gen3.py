...
import acts.examples.detray as detray_examples
...
from acts.examples.traccc import (
    TracccSeqAlgorithm, TracccMeasToActsAlg, TracccTrackToActsAlg,
    ActsMeasToTracccAlg,
)

from acts.examples.simulation import (
    MomentumConfig, EtaConfig, PhiConfig, ParticleConfig,
    ParticleSelectorConfig, addParticleGun, addFatras, addDigitization,
...
def makeGen3Digi():
    ...
    with open('Examples/Configs/odd-digi-geometric-config.json') as f:
        d1 = json.load(f)

    params = {e['volume']: e['value'] for e in d1['entries']}

    # Gen1 → Gen3 mapping
    pixel_val  = params[17]  # pixel barrel
    sstrip_neg = params[23]  # short strip endcap
    sstrip_bar = params[24]  # short strip barrel
    sstrip_pos = params[25]  # short strip endcap
    lstrip_neg = params[28]  # long strip endcap
    lstrip_bar = params[29]  # long strip barrel
    lstrip_pos = params[30]  # long strip endcap

    entries = []

    # Pixel barrel
    for v in [9, 10, 11, 12]:
        entries.append({'volume': v, 'value': pixel_val})

    # Short strip neg endcap
    for v in [21, 22, 23, 24, 25, 26, 27]:
        entries.append({'volume': v, 'value': sstrip_neg})

    # Short strip pos endcap
    for v in [36, 37, 38, 39, 40, 41, 42]:
        entries.append({'volume': v, 'value': sstrip_pos})

    # Short strip barrel
    for v in [48, 49, 50, 51]:
        entries.append({'volume': v, 'value': sstrip_bar})

    # Long strip neg endcap inner
    for v in [59, 60, 61, 62, 63, 64]:
        entries.append({'volume': v, 'value': lstrip_neg})

    # Long strip pos endcap inner
    for v in [72, 73, 74, 75, 76, 77]:
        entries.append({'volume': v, 'value': lstrip_pos})

    # Long strip barrel
    for v in [81, 82]:
        entries.append({'volume': v, 'value': lstrip_bar})

    # Long strip neg endcap outer
    for v in [90, 91, 92, 93, 94, 95]:
        entries.append({'volume': v, 'value': lstrip_neg})

    # Long strip pos endcap outer
    for v in [103, 104, 105, 106, 107, 108]:
        entries.append({'volume': v, 'value': lstrip_pos})

    config = {
        'acts-geometry-hierarchy-map': {'format-version': 0,
                                        'value-identifier': 'digitization-configuration'},
        'entries': entries
    }

    with open('Examples/Configs/odd-digi-geometric-config-gen3.json', 'w') as f:
        json.dump(config, f, indent=2)
    print(f'Written {len(entries)} entries')

def makeGen3Smearing():

    # Read Gen1 config to get smearing params
    d1 = json.load(open('Examples/Configs/odd-digi-smearing-config.json'))
    params = {e['volume']: e['value'] for e in d1['entries']}

    # Extract only the precise coordinate smearing for strips
    def strip_1d(vol_params):
        return {'smearing': [s for s in vol_params['smearing'] if s['index'] == 0]}

    pixel_smear  = params[17]  # 2D pixel barrel
    sstrip_smear = strip_1d(params[24])  # 1D strip only
    lstrip_smear = strip_1d(params[29])  # 1D strip only

    pixel_vols  = [9, 10, 11, 12]
    sstrip_vols = [21,22,23,24,25,26,27, 36,37,38,39,40,41,42, 48,49,50,51]
    lstrip_vols = [59,60,61,62,63,64, 72,73,74,75,76,77, 81,82, 90,91,92,93,94,95, 103,104,105,106,107,108]

    entries = []
    for v in pixel_vols:
        entries.append({'volume': v, 'value': pixel_smear})
    for v in sstrip_vols:
        entries.append({'volume': v, 'value': sstrip_smear})
    for v in lstrip_vols:
        entries.append({'volume': v, 'value': lstrip_smear})

    config = {
        'acts-geometry-hierarchy-map': {'format-version': 0, 'value-identifier': 'digitization-configuration'},
        'entries': entries
    }
    with open('Examples/Configs/odd-digi-smearing-config-gen3.json', 'w') as f:
        json.dump(config, f, indent=2)
...
def main():
    ...
    parser.add_argument("--digi-config", type=pathlib.Path, default=None)
    ...
    parser.add_argument("--conditions-file", type=pathlib.Path, default=None)
    ...
    oddDigiConfig = actsDir / "Examples/Configs/odd-digi-smearing-config-gen3.json"
    ...
    geoSelectionConfigFile = actsDir / "Examples/Configs/odd-seeding-config-gen3.json"
    stripGeoSelectionConfigFile = ""
    # stripGeoSelectionConfigFile = actsDir / "Examples/Configs/odd-strip-seeding-config-gen3.json"
